...
max_trace = 3
gamma_arr = 0.5**(np.arange(n_iter)+1) * (np.exp(1)-1)**(1-np.arange(n_iter))
points_denoised = optim.denoise_point_cloud_2(points_noisy, n_patch, patch_size, n_patch_neighbor,
                                          ...
                                          max_trace=max_trace, gamma=gamma_arr, ground_truth=np.array(bun_mesh.vertices))

pcd_denoised = o3d.geometry.PointCloud(o3d.utility.Vector3dVector(points_denoised))

noisy_distance = geometry.compute_distance_between_points(np.array(bun_mesh.vertices), points_noisy)
denoised_distance = geometry.compute_distance_between_points(np.array(bun_mesh.vertices), points_denoised)

print(np.mean(noisy_distance))
print(np.mean(denoised_distance))
